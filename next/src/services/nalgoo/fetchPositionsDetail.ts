...
export const fetchPositionsDetail = async (id: string | undefined | string[]) => {
  ...
  const response = await fetch(
    ...
    `${process.env.NALGOO_URL}/job-offer/${id}?expand=regions,employment_forms,positions,min_education,educations,workMode,position_type,business_unit,custom_fields&api_key=${process.env.NALGOO_API_KEY}`,
  )
