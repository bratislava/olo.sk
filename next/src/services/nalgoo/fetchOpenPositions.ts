...
export const fetchOpenPositions = async () => {
  ...
  const response = await fetch(
    ...
    `${process.env.NALGOO_URL}/job-offer?expand=company_sectors,regions,employment_forms,position_type,business_unit,custom_fields&api_key=${process.env.NALGOO_API_KEY}`,
  )
