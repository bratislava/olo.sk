...
const BranchesMapSection = ({ section }: Props) => {
  ...
  const { title, text, branches } = section ?? {} // note: either grab, or empty object
  ...
  const [hoveredBranchSlug, setHoveredBranchSlug] = React.useState<string | null>(null)
        ...
        <div className="h-[480px] w-full overflow-hidden">
          ...
          >
            {filteredBranches.map((branch) => {
              const { latitude, longitude } = branch.attributes ?? {}

              console.log(`${latitude} ${longitude} branch`)
                ...
                return (
                  <Marker key={branch.id} longitude={longitude} latitude={latitude} anchor="bottom">
                    <motion.button
                      style={{ originY: 1 }}
                      initial={{ scale: 0 }}
                      animate={{ scale: hoveredBranchSlug === branch.attributes?.slug ? 1 : 0.75 }}
                      whileTap={{ scale: 0.8 }}
                    >
                      <Link
                        href={getFullPath(branch) ?? ''}
                        onMouseEnter={() => setHoveredBranchSlug(branch.attributes?.slug ?? '')}
