...
Then use "Default Admin API Key" for strapi in local strapi instance in `http://localhost:1337/admin/plugins/meilisearch` tab Settings as `Meilisearch API Key` and "Default Search API Key" in `next/.env.local` file as `NEXT_PUBLIC_MEILISEARCH_SEARCH_API_KEY`.
